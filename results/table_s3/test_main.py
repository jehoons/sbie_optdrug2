...
def test_simulate(with_small, force):
    ...
    found_dict = {}
    targetting_table = pd.DataFrame([], columns=[
            'compound', 'target', 'model_variable', 'score'])
    i = 0 
        ...
        for t0 in targets:
            ...
            if t0 in fumia_nodes:                
                ...
                for node in model_nodes: 
                    if node == 'State_RTK' and skip_RTK == True:
                        continue 
    ...
    targetting_table_updated = targetting_table.groupby(['compound',
            'model_variable']).mean().reset_index()
    
    targetting_table_updated2 = targetting_table_updated.groupby('compound'
            )['model_variable'].apply(lambda x: returnThresh(x))
...
def test_postproc_1(with_small, force):
    ...
    ccle_dose_resp_sel = ccle_dose_resp.merge(cells_df, how='inner',
            left_on='Primary Cell Line Name', right_on='Cell line primary name' )
    ...
    ccle_dose_resp_sel_2 = ccle_dose_resp_sel.merge(ccle_drug_chemblid, how='inner',
            left_on='Compound', right_on='compound_name')

    ccle_dose_resp_sel_3 = ccle_dose_resp_sel_2.merge(pert_res, how='inner', 
            left_on='compound_chembl_id', right_index=True)

    ccle_dose_resp_sel_3.to_csv(chk_fumia_with_ccle_data, index=False)

    ccle_dose_resp_sel_3.groupby('Compound').mean().to_csv(
            chk_fumia_with_ccle_data_cell_mean)


def test_postproc_2(with_small, force):
    
    df = pd.read_csv(chk_fumia_with_ccle_data_cell_mean)
    df[['ActArea', 'A', 'P', 'Q']].corr().to_csv(output_corr)
...
def test_graph_generator(with_small, force):
    ...
    plt.savefig(fig_actarea_a); plt.close()
    
    df0.plot(x='ActArea', y='P', kind='scatter'); 
    plt.savefig(fig_actarea_p); plt.close()
    
    df0.plot(x='ActArea', y='Q', kind='scatter'); 
    plt.savefig(fig_actarea_q); plt.close()
...
chk_fumia_with_ccle_data_cell_mean = 'chk-drugwise-ccle-vs-fumia.csv'

output_corr = 'output-a1-corr.csv'

fig_actarea_a = 'figure-a1-act-vs-a.png'
fig_actarea_p = 'figure-a1-act-vs-p.png'
fig_actarea_q = 'figure-a1-act-vs-q.png'
